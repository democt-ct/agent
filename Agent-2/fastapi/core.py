...
AMAP_DEFAULT_CITY = _resolve_config("AMAP_DEFAULT_CITY", "AMAP_CITY", "")
AMAP_DEFAULT_CENTER = _parse_center(
    _resolve_config("AMAP_DEFAULT_CENTER", "AMAP_CENTER", ""),
    [],
)
...
_city_center_cache: Dict[str, Optional[Dict[str, float]]] = {}


async def _geocode_city_center(city: str) -> Optional[Dict[str, float]]:
    """Use AMap geocode API to resolve city name to center coordinates."""
    if not city:
        return None
    norm_city = _normalize_text(city)
    if norm_city in _city_center_cache:
        return _city_center_cache[norm_city]
    key = _get_amap_web_service_key()
    if not key:
        _city_center_cache[norm_city] = None
        return None
    try:
        async with httpx.AsyncClient(timeout=AMAP_SEARCH_TIMEOUT_SECONDS) as client:
        ...
        if geocodes:
            ...
            if loc and "," in loc:
                parts = loc.split(",")
                center = {"lng": float(parts[0]), "lat": float(parts[1])}
                _city_center_cache[norm_city] = center
                return center
    except Exception:
        pass
    _city_center_cache[norm_city] = None
...
async def _resolve_reference_center_async(requirement_payload: Dict[str, Any]) -> Optional[Dict[str, float]]:
    anchor = parse_amap_location(requirement_payload.get("anchor_location"))
    if anchor:
        return anchor
    city = _normalize_text(requirement_payload.get("city"))
    if city:
        center = await _geocode_city_center(city)
        if center:
            return center
    if AMAP_DEFAULT_CENTER:
        return {"lng": AMAP_DEFAULT_CENTER[0], "lat": AMAP_DEFAULT_CENTER[1]}
    return None
...
def interpret_requirement_payload(message: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        "city": _sanitize_city_name(city) or "",
...
def _merge_requirement_payload(
    ...
    if explicit_city:
        ...
        merged["city"] = previous_city or current.get("city") or ""
...
def _build_requirement_v2(requirement_payload: Dict[str, Any], memory_profile: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "city": _normalize_text(requirement_payload.get("city")) or "",
        "day_count": requirement_payload.get("day_count"),
        ...
        "poi_preference": requirement_payload.get("poi_preference"),
        "intent_clarity": requirement_payload.get("intent_clarity"),
        "follow_up_questions": list(requirement_payload.get("follow_up_questions") or []),
    }
...
def _build_structured_itinerary(
    ...
    requirement = requirement_override or interpret_requirement_payload(
        message,
        {"current_city": _extract_destination_hint(message) or ""},
    )
...
def _infer_planner_place_kind(name: str, category: str = "", query: str = "") -> str:
    ...
    if re.search(r"(公园|草坪|绿道|湿地)", source):
        return "park"
    # POI name is an admin area (township/district/village) — not a real place to visit
    if re.search(r"^.{1,4}(镇|乡|街道|片区|新区|开发区|高新区|经济区)$", _normalize_text(name)):
        return "admin_area"
    return "attraction"
...
def _fallback_city_candidate_queries(requirement_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    rows: List[Dict[str, Any]] = [
        ...
        {"query": f"{city}商场", "kind": "shopping", "reason": "补足下午到晚上的城市活力区域", "preferred_slots": ["下午", "晚上"]},
    ]
...
def _normalize_candidate_recall_entry(candidate: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Infer five-dimension tag
    from poi_tags import infer_poi_tags
    poi_tag = infer_poi_tags(
        name=query,
        category=candidate.get("category_hint") or "",
        query=query,
        legacy_place_kind=place_kind,
    )
    return {
        "query": query,
        "kind": place_kind,
        "poi_tag": poi_tag.dict(),
        "intro": _normalize_text(candidate.get("intro")),
...
def _build_candidate_recall_prompt(
    ...
    # ── Curation mode: filter from web-discovered places ─────
    if discovered_places:
        places_text = "\n".join(
            f"- {p.get('name', '')} ｜ 来源摘要：{p.get('source_snippet', '')[:100]}"
            for p in discovered_places[:40]
        )
        user_prompt = f"""你现在是一个旅游规划 Tool-using Agent 的“候选地点策展器”。
你的任务是从网络采集的真实推荐中，筛选出最适合用户需求的地点候选。

以下是网络上真实用户推荐的地点（从小红书、社交媒体和搜索引擎采集）：
{places_text}
...
{{
  ...
  "representative_candidates": [
    {{
      "query": "候选地点名",
      "kind": "attraction|museum|park|food_area|food_poi|shopping|night_view",
      "intro": "一句话介绍这个地点（如：藏在老城区的百年茶馆，本地人喝盖碗茶摆龙门阵的地方）",
      "reason": "为什么它适合这次行程",
      "source": "这条推荐的信息来源（如：小红书/搜索引擎/补充经典）",
...
策展规则：
1. 只从上述列表中筛选地点作为主候选，不要自己编造虚构地点。
2. 以城市经典必去地标为主（占 70-80%），辅以少量本地人推荐的小众发现（占 20-30%）。不要全都推荐小众冷门地点，主流景点才是行程骨架。
3. 过滤掉明显不是真实地点的条目（如“这不是景点”“怎么玩”“攻略”等泛文本）。过滤掉行政区划名和住宿类。
4. 优先选择有明确地点特征、来源摘要可信度高的条目。
5. 如果列表中缺少某个核心类别（如博物馆、美食片区），可以从你自己的知识中补充该城市必去的经典地标（不设数量上限，经典地标是主体），并在 reason 中注明“补充经典”。
6. 不要推荐咖啡店、酒吧等小类，除非用户明确提到。
7. `representative_candidates` 总数控制在 {candidate_target_min} 到 {candidate_target_max} 个之间。
8. 不要输出坐标，不要输出住宿。"""

        # Two-stage intent branching for curation mode
        if intent_result is not None:
            from intent_extractor import IntentClarity
            clarity = intent_result.clarity if hasattr(intent_result, 'clarity') else None
            if clarity == IntentClarity.VAGUE:
                user_prompt += "\n9. 用户需求尚未明确，优先选择口碑好、普适性强的候选。"
            elif clarity in (IntentClarity.PARTIAL, IntentClarity.CLEAR):
                pref_json = json.dumps(intent_result.preference.dict() if hasattr(intent_result.preference, 'dict') else {}, ensure_ascii=False)
                user_prompt += f"\n9. 用户偏好已明确：{pref_json}。请据此筛选匹配的地点，优先选择与偏好一致的候选。"
...
用户最新消息：{message}"""
        return [
            {
                "role": "system",
                "content": "你是中文旅游规划系统里的候选策展器，只输出合法 JSON。",
            },
            {
                "role": "user",
                "content": user_prompt,
    ...
    # ── Generation mode: LLM invents candidates from knowledge ───
...
2. 以城市经典地标为主体（地标、公园、博物馆、美食片区、夜景点），可少量加入本地人视角的隐藏发现（不超过 20%）。
3. 不要推荐咖啡店、酒吧等小类，除非用户明确提到。不要推荐泛类别词如”老街””步行街””商圈”，应推荐具体地名。
4. 对不够确定的餐饮/夜景类，可以给”可搜索词”，后续由工具验证。
5. `representative_candidates` 总数尽量控制在 {candidate_target_min} 到 {candidate_target_max} 个之间，保证热门代表性地点尽量齐全。
6. 不要输出坐标，不要输出虚构地点，不要输出住宿，不要输出行政区划名称（如镇、乡、街道）。
7. 如果用户指定 {day_count} 天，请保证候选池足够支撑 {day_count} 天规划。"""

    # Two-stage intent branching
    if intent_result is not None:
        from intent_extractor import IntentClarity
        clarity = intent_result.clarity if hasattr(intent_result, 'clarity') else None
        if clarity == IntentClarity.VAGUE:
            user_prompt += "\n8. 用户需求尚未明确，只推荐城市核心地标和必去景点，不要推荐小众、咖啡馆、酒吧等窄类地点。"
        elif clarity in (IntentClarity.PARTIAL, IntentClarity.CLEAR):
            pref_json = json.dumps(intent_result.preference.dict() if hasattr(intent_result.preference, 'dict') else {}, ensure_ascii=False)
            user_prompt += f"\n8. 用户偏好已明确：{pref_json}。请据此筛选和优先推荐匹配的地点。"
...
async def _generate_candidate_recall(
    ...
    # ── Social discovery: search web for real place recommendations ──
    discovered_places: Optional[List[Dict[str, Any]]] = None
    social_discovery_status: Optional[str] = None
    city = _normalize_text(requirement_payload.get("city")) or ""
    if city:
        try:
            from social_discovery import discover_places_from_social
            interests = requirement_payload.get("must_have") or []
            trip_style = _normalize_text(requirement_payload.get("trip_style")) or ""
            discovery_result = await discover_places_from_social(
                city=city,
                interests=list(interests) if interests else None,
                trip_style=trip_style,
            )
            social_discovery_status = discovery_result.get("status", "failed")
            if discovery_result.get("places"):
                discovered_places = discovery_result["places"]
        except Exception:
            social_discovery_status = "error"

    # ── LLM candidate recall (curation mode if discovery succeeded) ──
    try:
        content = await _request_text_completion(
            _build_candidate_recall_prompt(
                message, conversation_context, requirement_payload,
    ...
    def _dedup_key(query: str) -> str:
        """去掉城市名前缀后的标准化 key，用于跨来源去重（如"绵阳博物馆"和"博物馆"视为同一）"""
        q = _normalize_text(query)
        if city and q.startswith(city):
            q = q[len(city):]
        return q

    seen_queries = {_normalize_text(item.get("query")) for item in normalized_rows if _normalize_text(item.get("query"))}
    seen_dedup_keys = {_dedup_key(q) for q in seen_queries}
    ...
    return {
        ...
        "social_discovery_status": social_discovery_status,
        "social_discovery_place_count": len(discovered_places) if discovered_places else 0,
    }
...
def _decorate_validated_place(
    ...
    *,
    ...
    reason: str = "",
    poi_tag: Optional[Any] = None,
) -> Dict[str, Any]:
    # Re-infer kind from actual POI name/category to avoid inheriting
    # wrong kind from the candidate definition (e.g. query="绵阳商圈" kind=shopping
    # but POI returned is "青义" — a township, not a mall).
    poi_name = poi.get("name") or ""
    poi_category = poi.get("category") or ""
    re_inferred = _infer_planner_place_kind(poi_name, poi_category, query)
    actual_place_kind = re_inferred if re_inferred != "attraction" else (_normalize_text(place_kind) or "attraction")
    # If the actual POI is an admin area, always override — it's not a visitable place
    if re_inferred == "admin_area":
        actual_place_kind = "admin_area"
    # Infer five-dimension tag if not provided
    from poi_tags import infer_poi_tags as _infer_poi_tags
    if poi_tag is None:
        poi_tag = _infer_poi_tags(
            name=poi_name,
            category=poi_category,
            query=query,
            legacy_place_kind=actual_place_kind,
        )
    decorated = {
        "schema_version": PLANNER_SCHEMA_VERSION,
        ...
        "source_query": query,
        "poi_tag": poi_tag.dict() if hasattr(poi_tag, 'dict') else poi_tag,
    }
...
async def _resolve_candidate_pool(
    ...
    anchor_location = parse_amap_location(requirement_payload.get("anchor_location"))
    ref_center = await _resolve_reference_center_async(requirement_payload)
    ...
    for candidate_def in candidate_defs:
        ...
        if result.get("status") == "resolved" and isinstance(result.get("poi"), dict):
            ...
            # Skip admin-area POIs (townships, districts) — not real visitable places
            if poi.get("place_kind") == "admin_area":
                warnings.append(f"[admin_area_filter] {poi.get('name')} 是行政区划名称，不是可游玩地点，跳过")
                backups.append(
                    {
                        "query": candidate_def.get("query"),
                        "reason": f"{poi.get('name')} 是行政区划名称，不是可游玩地点",
                        "poi": poi,
                    }
                )
                continue
            poi_loc = parse_amap_location(poi.get("location"))
            if ref_center and poi_loc:
                dist_to_center = _distance_between_points(ref_center, poi_loc)
                if dist_to_center is not None and dist_to_center > MAX_SUBURB_DISTANCE_METERS:
                    warnings.append(f"[suburb_filter] {poi.get('name')} 距市中心 {dist_to_center/1000:.1f}km，超过 {MAX_SUBURB_DISTANCE_METERS/1000:.0f}km 限制，降级为备选")
                    backups.append(
                        {
                            "query": candidate_def.get("query"),
                            "reason": f"距市中心 {dist_to_center/1000:.1f}km，超过郊区限制",
                            "poi": poi,
                        }
                    )
                    continue
...
def _rank_validated_candidates_by_fivedim(
    ...
    """Rank validated candidates using five-dimension preference matching."""
    from poi_tags import infer_poi_tags, POITag
    from intent_extractor import (
        FiveDimPreference, derive_ranking_weights,
        jaccard, time_fit_score, budget_fit_score,
    )

    if not isinstance(preference, FiveDimPreference):
        return validated_candidates

    weights = derive_ranking_weights(preference)
    pref = preference

    def _score(item: Dict[str, Any]) -> float:
        poi = item.get("poi") or {}
        tag_dict = poi.get("poi_tag") or {}
        if tag_dict:
            tag = POITag(**tag_dict) if isinstance(tag_dict, dict) else POITag()
        else:
            tag = infer_poi_tags(
        ...
        func_match = 1.0 if tag.function.value in pref.function_weights else 0.0
        exp_overlap = jaccard(tag.experience, pref.experience_desired)
        crowd_match = 1.0 if any(c in tag.crowd for c in pref.crowd_required) else 0.0
        t_fit = time_fit_score(tag.time.best_time, pref.time_preference)
        b_fit = budget_fit_score(tag.budget.price_level, pref.budget_preference)
        landmark = 1.0 if tag.is_landmark else 0.0
        confidence = float(poi.get("confidence") or 0)

        return (
            weights["function_match"] * func_match
            + weights["experience_overlap"] * exp_overlap
            + weights["crowd_match"] * crowd_match
            + weights["time_fit"] * t_fit
            + weights["budget_fit"] * b_fit
            + weights["landmark_bonus"] * landmark
            + 0.3 * confidence
        )

    return sorted(validated_candidates, key=_score, reverse=True)


async def _supplement_validated_candidates(
    validated_candidates: List[Dict[str, Any]],
    ...
    for backup in ranked_backups:
        ...
        if not key or key in seen_keys:
            continue
        # Skip backups that were filtered for being too far from center
        if ref_center:
            poi_loc = parse_amap_location(poi.get("location"))
            if poi_loc:
                dist = _distance_between_points(ref_center, poi_loc)
                if dist is not None and dist > MAX_SUBURB_DISTANCE_METERS:
                    continue
        # Skip admin-area POIs (townships, districts) — not real visitable places
        if _normalize_text(poi.get("place_kind")) == "admin_area" or _infer_planner_place_kind(poi.get("name") or "", poi.get("category") or "") == "admin_area":
            continue
...
def _fallback_plan_from_validated_pool(
    ...
    def _pick_for_slot(slot: str) -> Optional[Dict[str, Any]]:
        best = None
        best_score = -1.0
        for item in pool:
            poi = item.get("poi") or {}
            ...
            if not poi_id or poi_id in used:
                continue
            score = _slot_suitability_score(slot, poi.get("place_kind") or "")
            ...
            if score > best_score:
                ...
                best_score = score
        if best:
            used.add(_normalize_text((best.get("poi") or {}).get("poi_id")))
        return best
...
5. 同一个 `poi_id` 不要重复进主行程。
6. 输出尽量顺路，不要明显来回跑，并优先保留城市关键地标。
...
def _enrich_itinerary_with_remaining_validated_candidates(
    ...
    def _fill_score(item: Dict[str, Any]) -> float:
        ...
        return {
            ...
            "cafe": 1.5,
            "bar": 1.3,
        }.get(place_kind, 1.0) + float(poi.get("confidence") or 0)
...
def _validate_and_repair_itinerary(
    ...
    city = _normalize_text(requirement_payload.get("city")) or _normalize_text(itinerary.get("city")) or ""
    trip_style = _normalize_text(requirement_payload.get("trip_style")) or "moderate"
...
async def _validate_existing_itinerary_draft(
    ...
    itinerary = {
        "schema_version": PLANNER_SCHEMA_VERSION,
        "city": _normalize_text(requirement_payload.get("city")) or _normalize_text(draft_itinerary.get("city")) or "",
...
async def _run_tool_using_chat_workflow(
    *,
    ...
    # ── Two-stage intent assessment ──
    from intent_extractor import assess_intent_clarity, IntentClarity
    intent_result = assess_intent_clarity(requirement_payload, message)
    workflow_trace.append(
        _make_workflow_trace_entry(
            "intent_assessment",
            start,
            details={"clarity": intent_result.clarity.value},
        )
    )
    ...
    if edit_instruction and latest_itinerary_payload:
        ...
        recall_payload = await _generate_candidate_recall(message, conversation_context, requirement_payload, intent_result=intent_result)
        assumptions = _normalize_text(recall_payload.get("assumptions"))
        candidate_recall_result = {
            "schema_version": PLANNER_SCHEMA_VERSION,
            "status": "ok",
            "assumptions": assumptions,
            "social_discovery_status": recall_payload.get("social_discovery_status"),
            "social_discovery_place_count": recall_payload.get("social_discovery_place_count"),
            "candidates": [
                {
        ...
        validated_candidates = await _supplement_validated_candidates(
            validated_candidates,
            candidate_backups,
            requirement_payload,
        )
        # ── Two-stage ranking ──
        if intent_result.clarity in (IntentClarity.CLEAR, IntentClarity.PARTIAL):
            validated_candidates = _rank_validated_candidates_by_fivedim(
                validated_candidates, intent_result.preference,
            )
        elif intent_result.clarity == IntentClarity.VAGUE:
            # Stage 1: filter to landmarks only (high confidence + known names)
            landmark_candidates = []
            for item in validated_candidates:
                poi = item.get("poi") or {}
                tag_dict = poi.get("poi_tag") or {}
                if tag_dict and tag_dict.get("is_landmark"):
                    landmark_candidates.append(item)
                elif float(poi.get("confidence") or 0) >= 0.85:
                    landmark_candidates.append(item)
            if landmark_candidates:
                validated_candidates = landmark_candidates
    ...
    # ── Reflexion: two-round quality + route validation loop ──
    from reflexion_engine import reflexion_loop as _reflexion_loop
    _reflexion_ref_center = await _resolve_reference_center_async(requirement_payload)
    reflexion_result = _reflexion_loop(
        itinerary_payload,
        validated_candidates,
        max_rounds=2,
        reference_center=_reflexion_ref_center,
    )
    itinerary_payload = reflexion_result
    reflexion_log = reflexion_result.get("reflexion_log") or []
    if reflexion_log:
        planner_warnings.extend(reflexion_log)
    if reflexion_result.get("reflexion_stable"):
        planner_warnings.append("[reflexion] 行程稳定，两轮验证通过")
    else:
        planner_warnings.append("[reflexion] 已达最大轮次，可能存在未修复项")
    ...
    # Append follow-up questions for Stage 1 (vague intent)
    if intent_result.follow_up_questions:
        follow_up = "\n\n".join(intent_result.follow_up_questions)
        assistant_text = assistant_text.rstrip() + "\n\n" + follow_up if assistant_text else follow_up
    return {
        "schema_version": PLANNER_SCHEMA_VERSION,
...
def _parse_session_row(row: sqlite3.Row) -> Dict[str, Any]:
    ...
    if "latest_user_message" in keys:
        result["latest_user_message"] = row["latest_user_message"]
...
def _apply_itinerary_edit_instruction(
    ...
    itinerary["city"] = _normalize_text(requirement_payload.get("city")) or itinerary.get("city") or ""
...
async def search_amap_poi(client: Any, task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    effective_keyword = keyword or city
    params: Dict[str, str] = {
        "key": key,
    ...
    if effective_mode == "nearby" and user_location:
        ...
        effective_city = city or region_name
        if effective_city:
...
async def api_planner_place_candidates(body: Any) -> Dict[str, Any]:
    ...
    city = _normalize_text(payload.get("city")) or ""
...
async def api_poi_resolve(body: Any) -> Dict[str, Any]:
    ...
    city = _sanitize_city_name(payload.get("city")) or ""
...
def debug_ui() -> FileResponse:
    debug_path = STATIC_DIR / "debug.html"
    if debug_path.exists():
        return FileResponse(str(debug_path))
    raise HTTPException(status_code=404, detail="static/debug.html not found")


def debug_pipeline_data(session_id: str) -> Dict[str, Any]:
    with get_db() as conn:
        _get_session_or_404(conn, session_id)
        req_payload = _get_latest_requirement_payload(conn, session_id)
        iti_row = _get_latest_itinerary_row(conn, session_id)
        messages = _list_messages(conn, session_id, limit=200)

    itinerary = None
    if iti_row:
        itinerary = _parse_itinerary_row(iti_row).get("content") or {}

    from reflexion_engine import has_food_interest as _has_food_interest

    return {
        "session_id": session_id,
        "requirement": {
            **(req_payload or {}),
            "has_food_interest": _has_food_interest(req_payload or {}),
        },
        "candidate_recall": itinerary.get("candidate_recall_result") if itinerary else {},
        "grounded_pois": itinerary.get("grounded_pois") if itinerary else [],
        "itinerary": itinerary or {},
        "reflexion_log": itinerary.get("reflexion_log") if itinerary else [],
        "reflexion_stable": itinerary.get("reflexion_stable") if itinerary else None,
        "planner_warnings": itinerary.get("planner_warnings") if itinerary else [],
        "workflow_trace": itinerary.get("workflow_trace") if itinerary else [],
        "messages": messages,
    }
...
async def chat(_body: Any) -> Dict[str, Any]:
    ...
    itinerary_payload = workflow_result.get("validated_itinerary") or {
        "city": requirement_payload.get("city") or "",
        "days": [],
    }
...
async def create_requirement(_session_id: str, _body: Any) -> Dict[str, Any]:
    ...
    with get_db() as conn:
        ...
        version = int(session["current_requirement_version"] or 0) + 1
        payload = structured_payload or interpret_requirement_payload(raw_input, {"current_city": ""})
        requirement_id = _new_id("req")
        created_at = _now_iso()
        conn.execute(
            """
...
async def preview_requirement_interpretation(_session_id: str, _body: Any) -> Dict[str, Any]:
    ...
    with get_db() as conn:
        _get_session_or_404(conn, _session_id)
    structured_payload = interpret_requirement_payload(raw_input, {"current_city": ""})
    return {
        "status": "ok",
...
async def create_itinerary(_session_id: str, _body: Any) -> Dict[str, Any]:
    ...
    with get_db() as conn:
        _get_session_or_404(conn, _session_id)
        latest_requirement_payload = _get_latest_requirement_payload(conn, _session_id) or {"city": ""}
        content = {
            "city": latest_requirement_payload.get("city") or "",
