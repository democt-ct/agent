...
class BudgetDimensionSchema(BaseModel):
    price_level: int = 2


class POITagSchema(BaseModel):
    function: str = "attraction"
    experience: List[str] = Field(default_factory=list)
    crowd: List[str] = Field(default_factory=list)
    time: Optional[TimeDimensionSchema] = None
    budget: Optional[BudgetDimensionSchema] = None
...
class PoiPlaceCandidate(BaseModel):
    ...
    poi_tag: Optional[POITagSchema] = None
...
class PlannerRequirementV2(BaseModel):
    ...
    poi_preference: Optional[Dict[str, Any]] = None
    intent_clarity: Optional[str] = None
