...
function scoreContentMention(candidate: CandidatePlace, signals: ContentSignal[] = []): number {
  ...
  for (const signal of signals) {
    ...
    if (signal.categoryHints.includes(candidate.category)) score += 4;
  }
  return Math.min(score, 24);
}

function hasTag(candidate: CandidatePlace, pattern: RegExp): boolean {
  return pattern.test(`${candidate.name} ${candidate.description ?? ""} ${candidate.tags.join(" ")}`);
}

export function isCoreRecommendablePoi(poi: CandidatePlace): CorePoiDecision {
...
function suggestedRole(candidate: CandidatePlace): SignaturePoolItemSummary["suggested_role"] {
  if (candidate.category === "food") return "meal_stop";
  if (candidate.category === "mall") return "shopping_cluster";
  if (candidate.category === "nightview") return "night_cluster";
...
function toSummary(candidate: CandidatePlace, score: number, reason: string): SignaturePoolItemSummary {
  return {
    name: candidate.name,
...
export function buildCitySignaturePool(
  city: string,
  candidatePool: CandidatePool,
  requirement: StructuredPayload
): CitySignaturePool {
  const profile = candidatePool.preferenceProfile ?? resolvePreferenceProfile(requirement);
  ...
  const scored = candidatePool.candidates.map((candidate) => {
    ...
    const categoryBase =
      candidate.category === "landmark" || candidate.category === "museum" ? 60 :
      ...
      34;
    const famousBoost = /(著名|地标|必打卡|博物馆|古镇|老街|夜市|商圈|购物中心|mall|广场)/i.test(
      `${candidate.name} ${candidate.description ?? ""}`
    ) ? 10 : 0;
    const profileBoost =
      profile.preference_params.food_priority === "high" && candidate.category === "food" ? 8 :
  ...
  const backupTrips = scored
    ...
    .map((item) => ({
      ...
      suggested_role: "backup_day_trip" as const
    }));
  const avoidAsCore = scored
    ...
    .map((item) => ({
      ...
      suggested_role: item.core.suggested_role === "transport" ? "backup" as const : "backup" as const
    }));
...
export function checkCityCoverage(candidatePool: CandidatePool): CoverageCheckResult {
  ...
  if (!hasEnoughMustVisit) missing_items.push("must_visit_attractions");
  if (!hasFood) missing_items.push("famous_foods_or_food_areas");
  if (!hasNight) missing_items.push("night_options");
  if (!hasLocal) missing_items.push("local_experiences");

  const repair_actions = missing_items.map((item) =>
    item === "must_visit_attractions"
