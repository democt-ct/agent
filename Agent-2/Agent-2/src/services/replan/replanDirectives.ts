...
const CATEGORY_PATTERNS: Array<{ category: PlaceCategory; patterns: RegExp[] }> = [
...
const SLOT_PATTERNS: Array<{ slot: PlannerTimeSlot; patterns: RegExp[] }> = [
...
function uniqueCategories(values: PlaceCategory[] | undefined): PlaceCategory[] | undefined {
  if (!values?.length) return undefined;
  return Array.from(new Set(values));
}

function detectCategories(text: string): PlaceCategory[] {
  const result: PlaceCategory[] = [];
...
function detectSlot(text: string): PlannerTimeSlot | undefined {
  return SLOT_PATTERNS.find((item) => item.patterns.some((pattern) => pattern.test(text)))?.slot;
}

function detectDay(text: string): number | undefined {
...
function detectPreferredPace(text: string): PreferredPace | undefined {
  if (/轻松|不累|慢一点|休闲|宽松/u.test(text)) return "relaxed";
  if (/紧凑|多安排|高效|赶一点/u.test(text)) return "compact";
  if (/适中|正常节奏/u.test(text)) return "moderate";
  return undefined;
}

function detectDistanceTolerance(text: string): DistanceTolerance | undefined {
  if (/只在市区|仅市区|别太远|不要跑太远|少走路/u.test(text)) return "urban_only";
  if (/近郊|周边|附近/u.test(text)) return "nearby_ok";
  if (/远一点也行|距离灵活/u.test(text)) return "flexible";
  return undefined;
}

function mergeDayOverrides(existing: DayOverride[] = [], incoming: DayOverride[] = []): DayOverride[] {
...
function mergeReplaceTargets(existing: ReplaceTarget[] = [], incoming: ReplaceTarget[] = []): ReplaceTarget[] {
  return [...existing, ...incoming].slice(-8);
}

export function extractReplanDirectives(text: string): ReplanDirectives | undefined {
  const normalized = String(text ?? "").trim();
  if (!normalized) return undefined;

  const preferredCategories = detectCategories(
    normalized.match(/(?:多一点|更偏|优先|想要|改成)([^，。！？]+)/u)?.[1] ?? normalized
  );
  const avoidCategories = detectCategories(
    normalized.match(/(?:不要|别|少一点|减少|换掉)([^，。！？]+)/u)?.[1] ?? ""
  );
...
export function mergeReplanDirectives(
  existing?: ReplanDirectives,
  incoming?: ReplanDirectives
): ReplanDirectives | undefined {
...
function mergeInterestCategories(payload: StructuredPayload, categories: PlaceCategory[] | undefined): string[] | undefined {
  if (!categories?.length) return payload.interests;
  return Array.from(new Set([...(payload.interests ?? []), ...categories]));
}

export function applyReplanDirectivesToPayload(payload: StructuredPayload): StructuredPayload {
  const directives = payload.replan_directives;
  if (!directives) return payload;

  const next: StructuredPayload = { ...payload };
  const global = directives.global;
  if (global?.preferred_pace) next.preferredPace = global.preferred_pace;
  if (global?.distance_tolerance) next.distanceTolerance = global.distance_tolerance;
...
export function hasReplanDirectiveSignal(payload?: StructuredPayload): boolean {
  const directives = payload?.replan_directives;
  return Boolean(
    directives?.global ||
...
export function summarizeAppliedDirectiveChanges(payload?: StructuredPayload): string[] {
  const directives = payload?.replan_directives;
  if (!directives) return [];
  const parts: string[] = [];
  if (directives.global?.preferred_pace === "relaxed") parts.push("整体节奏更轻松");
  if (directives.global?.distance_tolerance === "urban_only") parts.push("优先市区内与近距离转场");
  ...
  for (const item of directives.day_overrides ?? []) {
    ...
    if (changes.length) parts.push(`${scope} ${changes.join("，")}`);
  }
  return parts;
}

export function getDayOverrideForSlot(
  directives: ReplanDirectives | undefined,
  day: number,
