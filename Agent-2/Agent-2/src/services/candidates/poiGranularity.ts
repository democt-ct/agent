...
import { isGenericQueryTerm, isRealPlaceName, hasValidPoiSource } from "./genericTermFilter";
...
const SCENIC_QUERY_CATEGORIES = new Set<PlaceCategory>([
  "landmark",
  "park",
  "museum",
  "nature",
...
const MAIN_ITINERARY_GRANULARITIES = new Set<PoiGranularity>([
  "attraction_level",
  "district_level",
  "food_level"
]);

function getRequirementText(requirement: StructuredPayload): string {
  return String(
...
export function inferPlanningMode(requirement: StructuredPayload): PlanningMode {
  const text = getRequirementText(requirement);
  if (includesAny(text, INTERNAL_ROUTE_PATTERNS)) return "attraction_internal_route";
  if (includesAny(text, CITY_TRIP_PATTERNS)) return "city_trip";
  return "city_trip";
}

function wantsShopping(requirement: StructuredPayload): boolean {
  const text = `${getRequirementText(requirement)} ${(requirement.interests ?? []).join(" ")}`;
  return includesAny(text, SHOPPING_PATTERNS);
}

function buildCandidateText(candidate: CandidatePlace): string {
  return [
    candidate.name,
    candidate.address,
...
function hasParentStructure(candidate: CandidatePlace): boolean {
  const text = buildCandidateText(candidate);
  return /[-·•()（）]/.test(candidate.name) || includesAny(text, INTERNAL_CONTEXT_PATTERNS);
}

function inferGranularity(candidate: CandidatePlace, planningMode: PlanningMode): PoiGranularity {
  const text = buildCandidateText(candidate);
  ...
  if (
    includesAny(text, INTERNAL_FACILITY_PATTERNS) &&
...
function inferPlannerCategoryFromGranularity(
  granularity: PoiGranularity,
  candidate: CandidatePlace
): PlannerCategory {
  if ((candidate.roles ?? []).includes("food") && (candidate.roles ?? []).includes("walk")) return "food_walk";
  if (granularity === "food_level") return "food";
  if (granularity === "district_level") return "walk";
  if (granularity === "attraction_level") return "scenic";
  if (candidate.category === "nightview") return "scenic";
  return "supplemental";
}

function extractParentName(candidate: CandidatePlace, knownParents: CandidatePlace[]): string | undefined {
  const text = buildCandidateText(candidate);
  ...
  const direct = candidate.name.match(
    /(.+?(?:景区|公园|博物馆|纪念馆|古镇|老街|步行街|商圈|街区|广场|湖|山|寺|楼|湿地|园区|Scenic Area|Park|Museum|Old Street|Walking Street|District|Square|Lake|Mountain|Temple))(?:内)?[-·•\s()（）].+/i
  );
  if (direct?.[1]) return direct[1].trim();

  const context = text.match(
    /(.+?(?:景区|公园|博物馆|纪念馆|古镇|老街|步行街|商圈|街区|广场|湖|山|寺|楼|湿地|园区|Scenic Area|Park|Museum|Old Street|Walking Street|District|Square|Lake|Mountain|Temple))(?:内|里面| inside)/i
  );
  return context?.[1]?.trim();
}

function semanticAreaKey(name: string): string {
  return normalizeText(
    name.replace(/美食街|小吃街|老街|步行街|夜市|商圈|街区|滨江路|古城|食街|美食城|food street|food market|old street|walking street|district|night market|waterfront/gi, "")
  );
}

function semanticNightKey(name: string): string {
  return normalizeText(
    name.replace(/夜景|夜游|观景|night view|night|view|walk|stroll|citywalk/gi, "")
  );
}

function mergeCandidate(base: CandidatePlace, incoming: CandidatePlace): CandidatePlace {
  return {
    ...base,
...
function normalizeCandidate(
  ...
  if (!MAIN_ITINERARY_GRANULARITIES.has(granularity)) eligible = false;
  if (granularity === "district_level" && !representativeWalk) eligible = false;
  if (granularity === "food_level" && !(localFood || candidate.source === "llm_curator")) eligible = false;
  if (candidate.category === "mall" && !wantsShopping(requirement)) eligible = false;
  if (planningMode === "city_trip" && granularity === "internal_poi_level") eligible = false;
  if (granularity === "service_level" || granularity === "city_level") eligible = false;

  if (isGenericQueryTerm(candidate.name) || !isRealPlaceName(candidate.name)) {
    eligible = false;
  }
...
function rejectMerchantAlias(
  candidate: CandidatePlace,
  allCandidates: CandidatePlace[],
  planningMode: PlanningMode
): CandidatePlace {
  if (planningMode !== "city_trip") return candidate;
  const text = buildCandidateText(candidate);
  const queryCategories = candidate.queryCategories ?? [];
  const scenicOnlyTask =
  ...
  const scenicParent = allCandidates.find((item) => {
    ...
    if (itemGranularity !== "attraction_level" && itemGranularity !== "district_level") return false;
    const key = normalizeText(item.name);
  ...
  if (!shouldReject) return candidate;
...
function mergeInternalPois(
  candidates: CandidatePlace[],
  planningMode: PlanningMode
): {
  candidates: CandidatePlace[];
  parentInferredByTask: Map<string, string[]>;
} {
  ...
  const parentInferredByTask = new Map<string, string[]>();
  ...
  for (const candidate of candidates) {
    ...
    const existingParent =
      Array.from(merged.values()).find((item) => normalizeText(item.name) === normalizeText(parentName)) ??
    ...
    const parentGranularity = includesAny(parentName, DISTRICT_PATTERNS)
      ? "district_level"
      : "attraction_level";
    const parentCandidate: CandidatePlace = {
      ...candidate,
...
function mergeFoodWalkAreas(candidates: CandidatePlace[]): CandidatePlace[] {
  ...
  for (const candidate of candidates) {
    ...
    const peer = candidates.find((item) => {
      if (item.id === candidate.id || usedIds.has(item.id)) return false;
      if (item.granularity !== "district_level" && item.granularity !== "food_level") return false;
...
function linkNightViewCandidates(
  candidates: CandidatePlace[],
  queryPlan: CandidateQueryPlan | undefined
): {
  candidates: CandidatePlace[];
  linkedByTask: Map<string, string>;
  linkedNightView: boolean;
} {
  const linkedByTask = new Map<string, string>();
  ...
  for (const candidate of candidates) {
    if (candidate.category !== "nightview" || candidate.location) continue;
    const target = candidates.find((item) => {
      if (!item.location || item.id === candidate.id) return false;
      if (item.granularity !== "attraction_level" && item.granularity !== "district_level") return false;
      ...
      return Boolean(
        (itemKey && candidateKey && (candidateKey.includes(itemKey) || itemKey.includes(candidateKey))) ||
    ...
    const queryKey = (candidate.tags.find((tag) => tag.startsWith("origin_query:")) ?? "").replace("origin_query:", "");
    if (queryKey) linkedByTask.set(queryKey, target.name);
    enriched.push({
      ...candidate,
    ...
    for (const task of nightTasks) {
      const target = candidates.find((candidate) =>
        ...
        task.keywords.some((keyword) => {
          ...
          const candidateSemantic = semanticNightKey(candidate.name);
          return Boolean(
            (taskKey && candidateKey && (taskKey.includes(candidateKey) || candidateKey.includes(taskKey))) ||
...
function buildGroundingDiagnostics(params: {
  candidates: CandidatePlace[];
  queryTasks: CandidatePool["queryTasks"];
  parentInferredByTask: Map<string, string[]>;
  linkedByTask: Map<string, string>;
}): GroundingTaskDiagnostic[] {
  return params.queryTasks.map((task) => {
    ...
    const related = params.candidates.filter((candidate) =>
      candidate.tags.includes(`origin_query:${taskKey}`)
    );
    return {
      taskName: `${task.tool}:${task.keyword}`,
      primarySelected: uniqueStrings(
        related
...
export function normalizePoiGranularity(candidate: CandidatePlace, requirement: StructuredPayload): CandidatePlace {
  return normalizeCandidate(candidate, inferPlanningMode(requirement), requirement);
}

export function applyPoiGovernance(params: {
  candidates: CandidatePlace[];
  requirement: StructuredPayload;
  queryTasks: CandidatePool["queryTasks"];
  ...
  const governanceSummary: CandidateGovernanceSummary = {
    planningMode,
    ...
    hasFoodWalkMerge: linkedNightView.candidates.some((candidate) =>
      (candidate.roles ?? []).includes("walk") && (candidate.roles ?? []).includes("food")
    )
  };
