import type { CandidatePlace, CandidatePool, CandidateQueryPlan, PlaceCategory, StructuredPayload } from "../src/types";
import { applyPoiGovernance } from "../src/services/candidates/poiGranularity";
import { planDynamicItinerary } from "../src/services/planning/dynamicItineraryPlanner";

function assert(condition: unknown, message: string): void {
  if (!condition) throw new Error(message);
}

function makeCandidate(params: Partial<CandidatePlace> & Pick<CandidatePlace, "id" | "name" | "city" | "category">): CandidatePlace {
  return {
    source: "mcp_poi",
    tags: [],
...
function buildQueryPlan(destination: string, categories: PlaceCategory[]): CandidateQueryPlan {
  return {
    destination,
...
function govern(params: {
  ...
  const queryTasks = params.categories.map((category) => ({
    tool: "searchPOI" as const,
    city: String(params.requirement.destination ?? ""),
    keyword: category,
    category,
    priority: 5
  }));
  const governed = applyPoiGovernance({
...
function testCityTripDoesNotLeakInternalPoiIntoMainItinerary(): void {
  const requirement: StructuredPayload = {
  ...
  const planner = planDynamicItinerary({ requirement, candidatePool: pool });
...
function testInternalPoisMergeIntoParentSubStops(): void {
  const requirement: StructuredPayload = {
...
function testCitywalkAndFoodAreaCanMergeIntoFoodWalk(): void {
  const requirement: StructuredPayload = {
...
function testNightViewCanLinkToExistingArea(): void {
  const requirement: StructuredPayload = {
...
function testMerchantAliasFromScenicTaskIsRejected(): void {
  const requirement: StructuredPayload = {
...
function testAttractionInternalRouteAllowsInternalPoiPlanning(): void {
  const requirement: StructuredPayload = {
...
function printGovernanceSnapshot(): void {
  const requirement: StructuredPayload = {
  ...
  const linkedNightViewTarget = pool.groundingDiagnostics?.flatMap((item) =>
    item.linkedNightViewTarget ? [{ taskName: item.taskName, linkedNightViewTarget: item.linkedNightViewTarget }] : []
  ) ?? [];
  const parentInferred = pool.groundingDiagnostics?.flatMap((item) =>
    item.parentInferred.length ? [{ taskName: item.taskName, parentInferred: item.parentInferred }] : []
  ) ?? [];
  ...
  assert(
    planner.itinerary.days.every((day) => day.items.every((item) => item.granularity !== "internal_poi_level")),
    "snapshot should confirm no internal_poi_level enters city_trip main itinerary"
  );
}
