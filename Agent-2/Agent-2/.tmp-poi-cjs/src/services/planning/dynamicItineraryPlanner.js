...
exports.dedupePlannerCandidates = dedupePlannerCandidates;
exports.buildMapData = buildMapData;
exports.fallbackSelectWithoutCluster = fallbackSelectWithoutCluster;
exports.planDynamicItinerary = planDynamicItinerary;
const anchorClusterPlanner_1 = require("./anchorClusterPlanner");
const citySignaturePool_1 = require("./citySignaturePool");
...
function candidatePlannerCategory(candidate) {
    return candidate.plannerCategory ?? (0, anchorClusterPlanner_1.getPlannerCategory)(candidate);
}
function normalizePlannerCategoryLabel(value) {
    const text = normalizeText(value);
    if (/foodwalk|food_walk|walkfood/.test(text))
        return "food_walk";
...
function buildEffectiveDayTemplate(requirement, day, daysCount) {
    ...
    return {
        ...
        slots: base.slots.map((slot) => {
            const override = (0, replanDirectives_1.getDayOverrideForSlot)(requirement.replan_directives, day, slot.slot);
...
function dedupePlannerCandidates(candidates) {
    const result = [];
    const seen = new Set();
...
function getUserPreferenceScore(candidate, requirement, profile) {
    ...
    if (profile.preference_params.famous_spot_priority === "high" && plannerCategory === "scenic")
        score += 16;
    if (profile.preference_params.food_priority === "high" && plannerCategory === "food_walk")
        score += 12;
    if (profile.preference_params.hidden_gem_priority === "high" && (plannerCategory === "walk" || plannerCategory === "food_walk"))
        score += 12;
    if (profile.preference_params.walking_preference === "low" && (plannerCategory === "walk" || plannerCategory === "food_walk"))
        score -= 8;
...
function categoryRepeatPenalty(candidate, dayItems, tripCategoryUsage, requirement) {
    ...
    const tripPenalty = tripCount * (plannerCategory === "food" ? 2 : plannerCategory === "food_walk" ? 3 : 5);
...
function evaluateCandidateForSlot(params) {
    ...
    const directiveOverride = (0, replanDirectives_1.getDayOverrideForSlot)(params.requirement.replan_directives, params.day, params.slot.slot);
    const plannerCategory = candidatePlannerCategory(candidate);
...
function pickForSlot(params) {
    ...
    const dayOverride = (0, replanDirectives_1.getDayOverrideForSlot)(params.requirement.replan_directives, params.day, params.slot.slot);
...
function fallbackSelectWithoutCluster(params) {
    const warnings = ["fallback_select_without_cluster_used"];
    const selectable = dedupePlannerCandidates(params.candidates.map(anchorClusterPlanner_1.ensurePlannerEligibility)).filter(anchorClusterPlanner_1.isPlannerSelectable);
...
function planDynamicItinerary(input) {
    ...
    const preferenceProfile = input.candidatePool.preferenceProfile ?? (0, profileResolver_1.resolvePreferenceProfile)(input.requirement);
    const citySignaturePool = input.candidatePool.citySignaturePool ?? (0, citySignaturePool_1.buildCitySignaturePool)(String(input.requirement.destination ?? input.candidatePool.destination ?? ""), input.candidatePool, input.requirement);
    const coverageCheck = input.candidatePool.coverageCheck ?? (0, citySignaturePool_1.checkCityCoverage)({
        ...input.candidatePool,
        preferenceProfile,
        citySignaturePool
    });
    const travelPlan = (0, anchorClusterPlanner_1.buildTravelPlanningStructure)({
    ...
    const candidates = dedupePlannerCandidates(input.candidatePool.candidates.map(anchorClusterPlanner_1.ensurePlannerEligibility));
    ...
    const routeValidation = (0, routeValidator_1.validatePlannedRoute)({
        candidatePool: {
            ...input.candidatePool,
            preferenceProfile,
            citySignaturePool,
