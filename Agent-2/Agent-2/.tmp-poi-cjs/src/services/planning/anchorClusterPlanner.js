...
exports.getPlannerCategory = getPlannerCategory;
exports.ensurePlannerEligibility = ensurePlannerEligibility;
exports.isPlannerSelectable = isPlannerSelectable;
exports.buildCandidateDebugSummary = buildCandidateDebugSummary;
exports.buildTravelPlanningStructure = buildTravelPlanningStructure;
const replanDirectives_1 = require("../replan/replanDirectives");
function normalizeText(value) {
    return value.trim().toLowerCase().replace(/[^\p{L}\p{N}]/gu, "");
}
function getQualityFlags(candidate) {
    const raw = candidate.qualityFlags ?? candidate.quality_flags;
    return raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
}
function hasQualityFlag(candidate, flag) {
...
function getPlannerCategory(candidate) {
    ...
    const text = normalizeText(`${candidate.name} ${candidate.description ?? ""} ${candidate.tags.join(" ")} ${tier} ${granularity} ${candidate.roles?.join(" ") ?? ""} ${candidate.source}`);
...
function ensurePlannerEligibility(candidate) {
    ...
    const trustedCurator = candidate.source === "llm_curator" ||
    ...
    const inferredEligible = explicitEligible ??
...
function isPlannerSelectable(candidate) {
    ...
    const trustedCurator = candidate.source === "llm_curator" ||
        Boolean(qualityFlags.llm_curated) ||
        Boolean(qualityFlags.high_classicness);
    if (trustedCurator && groundingConfidence >= 0.55)
        return true;
    if (["core", "scenic", "walk", "food"].includes(tier) && groundingConfidence >= 0.55)
        return true;
    return !isLowValueMerchant(candidate);
}
function buildCandidateDebugSummary(candidates) {
    ...
    const groupBy = (selector) => candidates.reduce((acc, candidate) => {
...
function classifyIntent(requirement) {
    ...
    const text = normalizeText(`${interests.join(" ")} ${Array.isArray(requirement.constraints) ? requirement.constraints.join(" ") : ""}`);
...
function isLowValueMerchant(candidate) {
    const text = normalizeText(`${candidate.name} ${candidate.description ?? ""} ${candidate.tags.join(" ")}`);
    return /(?:酒店|宾馆|公寓|写字楼|办公楼|公司|超市|便利店|药店|银行|加油站|停车场|服务区|厕所|工厂|仓库|民宿|网吧|快递|诊所|医院|地产|中介|售票处|游客中心|车站)/u.test(text);
}
...
function intentBonus(candidate, intent) {
    const text = normalizeText(`${candidate.name} ${candidate.description ?? ""} ${candidate.tags.join(" ")}`);
    if (intent === "foodie" && /food|restaurant|meal|cafe/i.test(text))
        return 12;
    if (intent === "foodie" && /美食|小吃|餐饮|夜宵/u.test(text))
        return 12;
    if (intent === "photo" && /photo|photogenic|view/i.test(text))
        return 10;
    if (intent === "photo" && /拍照|打卡|出片|夜景/u.test(text))
        return 10;
    if (intent === "family" && /park|museum|landmark|family|kid/i.test(text))
        return 8;
    if (intent === "family" && /亲子|带娃|小朋友|儿童/u.test(text))
        return 8;
    if (intent === "culture" && /museum|culture|history/i.test(text))
        return 10;
    if (intent === "culture" && /博物馆|展览|文化|人文/u.test(text))
        return 10;
    if (intent === "nature" && /nature|scenic|park|mountain|lake/i.test(text))
        return 10;
    if (intent === "nature" && /自然|风景|山|湖|公园/u.test(text))
        return 10;
    if (intent === "light_trip" && candidate.category === "cafe")
        return 6;
    if (intent === "special_ops" && candidate.category === "landmark")
        return 6;
    return 0;
}
function qualityScore(candidate, intent) {
    ...
    if (candidate.category === "mall")
        score += 2;
    if (isLowValueMerchant(candidate))
        score -= 28;
    return Math.round(score);
}
function isAnchorEligible(candidate) {
    if (!candidate.location)
        return false;
    const plannerCategory = getPlannerCategory(candidate);
    if (isLowValueMerchant(candidate) && !(candidate.source === "llm_curator" || Boolean(getQualityFlags(candidate).llm_curated) || Boolean(getQualityFlags(candidate).high_classicness)))
        return false;
...
function isFoodEligible(candidate) {
    if (!candidate.location)
        return false;
    const plannerCategory = getPlannerCategory(candidate);
    if (isLowValueMerchant(candidate) && !(candidate.source === "llm_curator" || Boolean(getQualityFlags(candidate).llm_curated) || Boolean(getQualityFlags(candidate).high_classicness)))
...
function clusterRadius(candidate, intent) {
    const plannerCategory = getPlannerCategory(candidate);
    if (plannerCategory === "scenic" || intent === "nature")
        return 6500;
    if (plannerCategory === "walk")
        return 3200;
    if (plannerCategory === "food_walk")
        return 2500;
    if (plannerCategory === "food")
        return 1800;
    return 3000;
}
function foodClusterRadius(candidate) {
    const plannerCategory = getPlannerCategory(candidate);
    if (plannerCategory === "walk" || plannerCategory === "food_walk")
        return 2200;
    return 1800;
}
function summarizeCategoryCounts(candidates) {
...
function averageCenter(candidates) {
...
function chooseAnchorClusterType(memberCount, categories, categoryCounts, anchor) {
    const scenic = categories.has("scenic");
    const support = categoryCounts.food ?? 0;
    if (memberCount >= 4 && scenic && support >= 2 && categories.size >= 3 && (anchor.citySignatureScore ?? 0) >= 42)
        return "full_cluster";
    if (scenic && ((anchor.citySignatureScore ?? 0) >= 50 || anchor.confidence >= 0.8 || memberCount >= 2))
        return "single_anchor";
    return "weak_cluster";
}
function canBeDayAnchor(candidate, tripDays) {
...
function scoreAnchorCandidate(candidate, clusterSupportScore, routeFitScore) {
    const citySignatureScore = candidate.citySignatureScore ?? 0;
    const plannerCategory = getPlannerCategory(candidate);
    const representativeScore = plannerCategory === "scenic" ? 80 :
...
function inferClusterRole(categories) {
    if (categories.has("food"))
        return "food_cluster";
    if (categories.has("walk"))
        return "leisure_cluster";
    if (categories.has("scenic"))
        return "sightseeing_cluster";
    return "sightseeing_cluster";
}
function buildAnchorClusters(params) {
    ...
    for (const anchor of anchors) {
        if (usedAnchors.has(anchor.candidate.id))
            continue;
        ...
        const categories = new Set(clusterCandidates.map((candidate) => getPlannerCategory(candidate)));
...
function buildFoodClusters(params) {
    const anchors = params.candidates
    ...
    const clusters = [];
    for (const anchor of anchors) {
        if (usedMembers.has(anchor.candidate.id))
            continue;
        if (!anchor.candidate.location)
            continue;
        ...
        if (members.length < 2 || supportCount < 2)
            continue;
...
function findClusterById(clusters, clusterId) {
    if (!clusterId)
        return undefined;
...
function getClusterCandidates(cluster, candidateLookup, categories) {
    if (!cluster)
        return [];
...
function selectCandidateIds(candidates, limit) {
    return candidates.slice(0, limit).map((candidate) => candidate.id);
}
function pickNearbyFoodCluster(anchorCluster, foodClusters, usedClusterIds) {
    if (!anchorCluster?.center)
        return {};
    const ranked = foodClusters
        ...
        if (Math.abs(distanceDelta) > 1200)
    ...
    return {
        ...
        transferReason: picked.distance > 3000
            ? "景点周边餐饮支撑较弱，中午转去更成熟的美食片区，换取更稳定的用餐体验。"
            : "中午接入附近成熟餐饮片区，补足主景点周边的吃饭选择。"
    };
}
function pickNearbySupportCluster(anchorCluster, allClusters, usedClusterIds, categories, maxDistanceMeters) {
...
function buildFallbackCluster(candidates) {
    const top = candidates[0];
    if (!top)
        return undefined;
    const scoped = candidates.slice(0, 4);
    const categories = new Set(scoped.map((candidate) => getPlannerCategory(candidate)));
    return {
        id: `anchor_${top.id}`,
        anchorId: top.id,
        anchorName: top.name,
...
function buildSlotPlan(params) {
    return {
        ...
        segmentType: params.slot === "morning" ? "sightseeing" :
    ...
    for (let day = 1; day <= params.daysCount; day += 1) {
        ...
        if (mainCluster) {
            slotPlans.push(buildSlotPlan({
                ...
                rationale: mainCluster.type === "single_anchor"
        ...
        if (lunchCluster?.id && lunchCluster.id !== mainCluster?.id)
            usedClusterIds.add(lunchCluster.id);
        const lunchOverride = (0, replanDirectives_1.getDayOverrideForSlot)(params.requirement.replan_directives, day, "lunch");
        const lunchCategories = lunchOverride?.preferred_categories?.length
        ...
        const preferSameAnchorAfternoon = mainCluster?.type === "full_cluster" &&
            ["walk"].some((category) => mainCluster.categories.includes(category));
        ...
        const afternoonOverride = (0, replanDirectives_1.getDayOverrideForSlot)(params.requirement.replan_directives, day, "afternoon");
        const afternoonCandidates = getClusterCandidates(afternoonCluster, candidateLookup, afternoonOverride?.preferred_categories?.length
        ...
        slotPlans.push(buildSlotPlan({
            ...
            rationale: afternoonCluster?.id === mainCluster?.id
                ? "下午延续同片区轻松内容，把节奏从打卡切到逛和休息，控制疲劳度。"
                : "下午转到附近更适合休闲逛吃的片区，让整天从高强度观光自然过渡到轻松活动。"
        }));
        const eveningCluster = pickNearbySupportCluster(afternoonCluster ?? mainCluster, supportClusters, usedClusterIds, ["scenic", "food", "walk"], 9000) ??
            lunchCluster ??
            afternoonCluster ??
            mainCluster;
        ...
        const eveningOverride = (0, replanDirectives_1.getDayOverrideForSlot)(params.requirement.replan_directives, day, "evening");
        const eveningCandidates = getClusterCandidates(eveningCluster, candidateLookup, eveningOverride?.preferred_categories?.length
        ...
        const auxCluster = findClusterById(params.clusters, lunchCluster?.id && lunchCluster.id !== mainCluster?.id ? lunchCluster.id : afternoonCluster?.id);
        dayPlans.push({
            day,
...
function buildTravelPlanningStructure(params) {
    ...
    const warnings = [...params.candidatePool.warnings];
    ...
    if (!foodPool.length)
        warnings.push("no strong food pool candidates were found");
