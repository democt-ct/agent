...
exports.resolvePreferenceProfile = resolvePreferenceProfile;
...
function mergePreferenceParams(primary, secondary) {
    const fatigueOrder = ["very_low", "low", "medium", "high"];
    const levelOrder = ["low", "medium", "high"];
    const densityOrder = ["low", "low_to_medium", "medium", "high"];
    const minLevel = (values, left, right) => values[Math.min(values.indexOf(left), values.indexOf(right))];
    const maxLevel = (values, left, right) => values[Math.max(values.indexOf(left), values.indexOf(right))];
    return {
        ...
        max_major_transfers_per_day: Math.min(primary.max_major_transfers_per_day, secondary.max_major_transfers_per_day),
        schedule_density: minLevel(densityOrder, primary.schedule_density, secondary.schedule_density),
        walking_preference: minLevel(["low", "medium", "high"], primary.walking_preference ?? "medium", secondary.walking_preference ?? "medium")
    };
}
function resolvePreferenceProfile(payload) {
    const text = getText(payload);
    let primary = "light_comfort";
    let secondary = "classic_must_visit";
    const reasons = [];
