    ...
    function showTemporaryNotice(text, tone = "info") {
      ...
      el.className = [
        "fixed bottom-24 right-4 z-[60] max-w-[380px] rounded-2xl border px-4 py-3 text-sm shadow-2xl backdrop-blur-xl",
        tone === "error"
      ...
      el.innerHTML = `
        <div class="flex items-start gap-2">
          <div class="flex-1">${escapeHtml(text)}</div>
          <button type="button" class="shrink-0 rounded-full p-1 hover:bg-white/10" onclick="this.closest('#toastNotice')?.remove()">
    ...
    async function sendMessage(text, options = {}) {
      ...
      showTyping(true);
      setWorkbenchStatus("正在理解你的需求...");

      const sessionId = state.session?.id || undefined;
      const loadingTimeout = setTimeout(() => {
        if (state.typing) {
          setWorkbenchStatus("正在搜索地点和规划路线，请稍候...");
        }
      }, 8000);
      ...
      try {
        ...
        if (bundle.action === "planning_error") {
          showTemporaryNotice("行程规划遇到问题，请尝试更简短的需求描述，或告诉我具体想去的地方。", "error");
        } else if (bundle.warnings?.length) {
          showTemporaryNotice(`行程已生成，但有 ${bundle.warnings.length} 个注意事项，请查看详情。`, "info");
        }
      } catch (error) {
        clearTimeout(loadingTimeout);
        const errorMsg = error?.message || "";
        if (errorMsg.includes("timeout") || errorMsg.includes("Timeout")) {
          showTemporaryNotice("请求超时，服务器可能繁忙，请稍后重试。", "error");
          setWorkbenchStatus("请求超时，请重试。");
        } else if (errorMsg.includes("500") || errorMsg.includes("internal")) {
          showTemporaryNotice("服务器内部错误，请稍后重试。", "error");
          setWorkbenchStatus("服务器错误，请重试。");
        } else if (errorMsg.includes("404") || errorMsg.includes("not found")) {
          showTemporaryNotice("会话未找到，已为你创建新会话。", "error");
          prepareNewSession();
        } else {
          showTemporaryNotice("发送失败，请检查网络后重试。", "error");
          setWorkbenchStatus("发送失败，请重试。");
        }
