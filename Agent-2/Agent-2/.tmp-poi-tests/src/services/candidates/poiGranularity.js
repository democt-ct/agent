...
const INTERNAL_ROUTE_PATTERNS = [
    /景区内部|公园内|园区内|内部怎么逛|内部路线|景区内路线|园区内怎么走/i
];
...
export function inferPlanningMode(requirement) {
...
function linkNightViewCandidates(candidates, queryPlan) {
    ...
    for (const candidate of candidates) {
        ...
        const target = candidates.find((item) => {
            ...
            return itemKey && candidateKey && (candidateKey.includes(itemKey) || itemKey.includes(candidateKey));
        });
        if (!target)
            continue;
        ...
        for (const task of nightTasks) {
            const target = candidates.find((candidate) => candidate.location &&
                ...
                task.keywords.some((keyword) => {
                    ...
                    const candidateKey = normalizeText(candidate.name);
                    return taskKey && candidateKey && (taskKey.includes(candidateKey) || candidateKey.includes(taskKey));
                }));
            if (!target)
                continue;
            linkedNightView = true;
...
export function normalizePoiGranularity(candidate, requirement) {
