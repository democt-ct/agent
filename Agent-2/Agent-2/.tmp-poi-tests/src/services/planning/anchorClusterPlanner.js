...
export function getPlannerCategory(candidate) {
    const explicit = String(candidate.plannerCategory ?? "").trim().toLowerCase();
    if (explicit === "scenic" || explicit === "food" || explicit === "walk" || explicit === "supplemental") {
        return explicit;
    }
    ...
    if (granularity === "food_level" || /foodwalk/.test(text)) {
        return (candidate.roles ?? []).includes("walk") ? "walk" : "food";
    }
...
export function ensurePlannerEligibility(candidate) {
...
export function isPlannerSelectable(candidate) {
...
export function buildCandidateDebugSummary(candidates) {
...
function isAnchorEligible(candidate) {
    ...
    return ["scenic", "walk", "food"].includes(plannerCategory);
}
